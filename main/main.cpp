...
extern "C" void app_main(void)
{
    ...
    if (state_buffer_len != sizeof(state_buffer) || memcmp(state_buffer_r, state_buffer, sizeof(state_buffer)) != 0) {
        printf("FAIL: STATE_BUFFER mismatch\n");
        verification_passed = false;
    }
    if (lora_key_len != sizeof(lora_key) || memcmp(lora_key_r, lora_key, sizeof(lora_key)) != 0) {
